...
local function processPart(part)
    pcall(function()
        ...
        if isBaseplate(part) then
            -- Lower the terrain visually by 1.5 studs so it doesn't clip above the floor
            workspace.Terrain:FillBlock(part.CFrame * CFrame.new(0, -1.5, 0), part.Size, Enum.Material.Sand)
...
if not foundAny then
    workspace.Terrain:FillBlock(CFrame.new(0,-2,0), Vector3.new(2000, 4, 2000), Enum.Material.Sand)
end
...
print("[Terrain] Successfully applied Sand terrain (StreamingEnabled supported)!")
